...
public class SceneCmdData {
    ...
    public static byte[] sendAddSceneCmd(String scenename, int group_id,int dev_count,String merge_mac) {
        ...
        byte[] id_data = FtFormatTransfer.byteMerger(bt_send_data, group_id_bt);

        //设备个数及其mac地址数据
        byte[] count_mac = FtFormatTransfer.byteMerger(bt_dev_count,bt_dev);
        byte[] bt_merge = FtFormatTransfer.byteMerger(id_data,count_mac);

        byte[] crc_bt = null;
        if (!Utils.isCRC8Value(Utils.CrcToString(bt_merge, bt_merge.length))) {
            String hex = Utils.StringToHexString(Utils.CrcToString(bt_merge, bt_merge.length));
            crc_bt = Utils.HexString2Bytes(hex);
        } else {
            crc_bt = Utils.HexString2Bytes(Utils.CrcToString(bt_merge, bt_merge.length));
        }

        //Cmd 数据与CRC8相加
        byte[] bt_send_cmd = FtFormatTransfer.byteMerger(bt_merge, crc_bt);
    ...
    public static byte[] sendDeleteSceneCmd(int scene_id) {
        ...
        if (!Utils.isCRC8Value(Utils.CrcToString(bt_send, bt_send.length - 1))) {
            String ss = Utils.StringToHexString(Utils.CrcToString(bt_send, bt_send.length - 1));
            bt_send[34] = Utils.HexString2Bytes(ss)[0];
        } else {
            bt_send[34] = Utils.HexString2Bytes(Utils.CrcToString(bt_send, bt_send.length - 1))[0];
        }
