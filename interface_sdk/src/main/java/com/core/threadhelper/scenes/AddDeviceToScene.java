 ...
 */
public class AddDeviceToScene implements Runnable {
    public final String TAG = "AddDeviceToScene";
    ...
    public AddDeviceToScene(Context context, AppDevice appDevice, short group_id, short scene_id) {
    ...
    public void run() {
        try {
            ...
            if (GW_IP_ADDRESS.equals("")) {//!NetworkUtil.NetWorkType(mContext)
                ...
                Log.i(TAG + "当前发送数据",TransformUtils.binary(bt_send, 16));
...
//                Constants.sendMessage(store_scene);
                ...
                Log.i(TAG + "接收数据 = ",Arrays.toString(recbuf));
            }
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
