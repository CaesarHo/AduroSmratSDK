...
public class ParseDeviceData {
    ...
    public static void ParseGetDeviceInfo(byte[] bt, boolean isNew_Device) throws Exception {
        ...
        if (!dev_name.equalsIgnoreCase("Device")) {
            ...
            device_name = Utils.toStringHex2(device_name_hex);
        } else {
            device_name = "Device";
...
//            System.out.println(a + " 是一个mac地址！");
        ...
        if (device_id.equalsIgnoreCase("ffff")) {
            ...
            byte[] bt_send = DeviceCmdData.ActiveReqDeviceCmd(device_mac, device_shortaddr, main_endpoint);
        ...
        public void parseBytes(byte[] data) {
            ...
            for (int i = 0; i < mDevMac_bt.length; i++) {
                if (mDevMac_bt[i] >= 0 && mDevMac_bt[i] <= 16) {
                    if (mDevMac_bt[i] >= 10 && mDevMac_bt[i] <= 16) {
                        str_zero = "0" + Integer.toHexString(mDevMac_bt[i] & 0xFF);
                    } else {
    ...
    public static class ParseAttributeData {
        ...
        public void parseBytes(byte[] data) {
            ...
            for (int i = 0; i < shortaddress.length; i++) {
                ...
                if (shortaddress[i] >= 0 && shortaddress[i] <= 16) {
                    shortaddr_zero = "0" + (shortaddress[i] & 0xFF);
                    shortaddr = shortaddr + shortaddr_zero;
                } else {
                    shortaddr = shortaddr + Integer.toHexString(shortaddress[i] & 0xFF);
                }
            }
            System.out.println("ParseAttributeData shortAddr = " + shortaddr);
